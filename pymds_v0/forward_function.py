...
def mds_new(seismic_scenario, scaling_factors, constants, parameters, long_int, seis_int):
    ...
    for j in range (0, len(data)) : # loop over samples
        ...
        dpj[62] = ((dpj[62])*sin((beta - alpha)*pi/180))+(start_depth*sin((beta - alpha)*pi/180)*rho_coll)  # in the direction perpendicular to colluvium surface
    ...
    """ VARIABLES INITIALIZATION FOR SEISMIC PHASE"""
    ...
    for iseg in range (1, N_eq):
        ...
        for k in range (0, len(j)) :                                                  
            ...
            djk = d[j[k],:]
            djk[62] = djk[62]*sin((beta - alpha)*pi/180)
            ...
            for l in range (0, iseg):                                                     
                ...
                djk[62] = djk[62] - (slip[l]*rho_coll*sin((beta - alpha) *pi/180)) # depth (along z) are modified after each earthquake
                d0 = djk.copy() # .copy() necéssaire
                d0[62] = 0 
...
def mds_torch_nov(seismic_scenario, scaling_factors, constants, parameters, long_int=100, seis_int=100, find_slip=False):
    ...
    for k in range (0, len(j1)):
        ...
        for ii in range (0, int(age[0]/time_interval)+1):
            ...
            P_cosmo, P_rad = clrock(djk, ejk, Lambda_f_e, so_f_e, EL_f[1], EL_mu[1]) 
            scorr = S_S[int(hjk)]/S_S[0]      #!!! différent surface scaling factor (scorr)
