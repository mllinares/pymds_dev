...
seismic_interval = 500 # interval used in seismic sequence (yr)
...
allow_rpt_incertitude = False # Use normal distribution on break-ups
...
for i in range (0, len(post_ages70)):
    ...
    if invert_sr == True:
        ...
        inferred_scenario70['SR'] = torch.tensor(true_scenario['SR'])
    cl36_models_post70[:,i]=forward.mds_torch(inferred_scenario70, scaling_factors, constants, parameters, long_int=500, seis_int=500, find_slip = invert_slips)
